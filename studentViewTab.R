...
studentViewTabServer <- function(id, uploadedData) {
  moduleServer(id, function(input, output, session) {
    ...
    output$trsTotalBox <- renderValueBox({
      value <- selectedStudent$data$trsTotalBehavior
      if(value <= 24 )
        valueBox(
          value,
          "TRS Total",
    ...
    output$trsSocialBox <- renderValueBox({
      value <-selectedStudent$data$trsSocialBehavior
      if(value <= 7)
        valueBox(
          value,
          "TRS Social",
    ...
    output$trsAcademicBox <- renderValueBox({
      value <-selectedStudent$data$trsAcademicBehavior
      if(value <= 5)
      valueBox(
        value,
        "TRS Academic",
    ...
    output$trsEmotionalBox <- renderValueBox({
      value <- selectedStudent$data$trsEmotionalBehavior
      if (value <= 11)
        valueBox(
          value,
          "TRS Emotional",
    ...
    output$myTotalBox <- renderValueBox({
      value <-selectedStudent$data$totalBehavior
      if (value <=23)
        valueBox(
          value,
          "Total",
    ...
    output$mySocialBox <- renderValueBox({
      value <- selectedStudent$data$socialBehavior
      if (value <= 9 && value >= 0)
        valueBox(
          value,
          "Social",
    ...
    output$myEmotionalBox <- renderValueBox({
      value<-selectedStudent$data$emotionalBehavior
      if (value <= 7 && value >= 0)
        valueBox(
          value,
          "Social",
    ...
    output$myAcademicBox <- renderValueBox({
      value <-selectedStudent$data$academicBehavior
    ...
    output$trsTotalBar <- renderPlot({
      ...
      ggplot(df_abrange, aes(x = ranges, y = n)) +
        ...
        scale_fill_manual(values = c("#FB8072", "#FFC000", "#00A300")) +
        theme_hc() +
